...
import oregano.internal.Regex.*
import oregano.internal.QuantifierType.*
...
object parser {
  ...
  def parse[Err: ErrorBuilder](s: String): Result[Err, Regex] = {
    regex.parse(s)
  }
  ...
  private lazy val positiveLookahead = AheadPos(atomic("(?=") ~> expr <~ ')')
  private lazy val negativeLookahead = AheadNeg(atomic("(?!" ~> expr <~ ')'))
  private lazy val positiveLookbehind = BehindPos(atomic("(?<=") ~> expr <~ ')')
  private lazy val negativeLookbehind = BehindNeg(atomic("(?<!" ~> expr <~ ')'))
  private lazy val independent = Atomic(atomic("(?>") ~> expr <~ ')')
  ...
  private lazy val backreference = Back(atomic('\\' ~> int))
